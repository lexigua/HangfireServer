(function ($) {
    ...
    $.easyui = (function () {
        ...
        return {
            ...
            //显示全屏加载进度条
            showLoadingByStr: function (str) {
                var $body = $("body");
                $("<div id=\"div_loading\" class=\"datagrid-mask\"></div>").css({ zIndex: "99998", display: "block" }).appendTo($body);
                $("<div id=\"div_loading_msg\" class=\"datagrid-mask-msg\"></div>").html(str).appendTo($body).css({ zIndex: "99999", display: "block", left: "50%", fontSize: "12px" });
            },
            //移除全屏加载进度条
            ...
            addIframeToTabs: function (tabsId, title, url, icon, closable) {
                ...
                if (!title && !url)
                    return;
                var tabs = $.easyui.getById(tabsId);//$('#' + tabsId);
                var index;
                var iframe = null;
                if (!exists())
                    createTab();
            ...
            warn: function (msg, title) {
                ...
                if (!msg)
                    return;
                getMessager().alert(title || "提示", msg, 'error');
            },
